...
public class OceanStats
{
    ...
    public boolean isViable(Ocean ocean)
    {
        ...
        nonZero = counters.keySet().stream().map((key) -> counters.get(key)).filter((info) -> (info.getCount() > 0)).map((_item) -> 1).reduce(nonZero, Integer::sum);
        //System.out.println("Numero de classes detectadas = " + nonZero);
        return nonZero > 1;
    }
